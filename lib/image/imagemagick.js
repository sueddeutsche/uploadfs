...
module.exports = function() {
  return {
    ...
    convert: function(context, callback) {
      ...
      // We need to treat animated gifs differently
      if (context.info.animated === true && context.extension === 'gif') {
        return createCopysForAllSizes(context, callback);
      }

      // No need for a separate animated GIF strategy after all thanks
      // to the -clone 0--1 option which preserves all animation frames
      return convertFast(context, callback);


      function createCopysForAllSizes(context, callback) {
        var imageSizes = [{ name: 'original' }].concat(context.sizes);
        ...
        _.each(imageSizes, function (size) {
          ...
          childProcess.execFile('cp', args, function(err) {
            if (err) {
              console.log(err);
            }
          });
